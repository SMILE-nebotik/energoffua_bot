...
async def get_all_users():
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute('SELECT user_id, group_number, alert_time FROM users') as cursor:
            return await cursor.fetchall()

# графіки засейвлені
async def save_schedule_cache(date_str: str, schedule_dict: dict):
    async with aiosqlite.connect(DB_NAME) as db:
        for group, hours in schedule_dict.items():
            await db.execute('''
                INSERT INTO schedule_cache (date, group_code, hours_data)
                VALUES (?, ?, ?)
                ON CONFLICT(date, group_code) DO UPDATE SET 
                hours_data = excluded.hours_data,
                updated_at = CURRENT_TIMESTAMP
            ''', (date_str, group, json.dumps(hours)))
        await db.commit()

async def get_schedule_for_group(date_str: str, group: str):
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute('SELECT hours_data FROM schedule_cache WHERE date = ? AND group_code = ?', (date_str, group)) as cursor:
